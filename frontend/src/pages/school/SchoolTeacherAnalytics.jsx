...
export default function SchoolTeacherAnalytics() {
      ...
      try {
        ...
        setError(
          err?.response?.data?.error ||
            ...
            "Failed to load teacher monitoring."
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <SchoolPageShell
      title="Teachers"
      subtitle="Monitor teacher assignments, class averages, and student progress across your school."
    >
      {loading ? (
        <p className="text-emerald-800">Loading teacher monitoring...</p>
      ) : error ? (
        ...
        <div className="space-y-6">
          <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            ...
            <div className="rounded-3xl border border-emerald-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-bold uppercase tracking-wide text-gray-500">School Class Avg</p>
          ...
          <section className="rounded-3xl border border-emerald-200 bg-white p-5 shadow-sm">
            <h2 className="text-lg font-black text-emerald-950">Teacher Monitoring</h2>
            {teachers.length === 0 ? (
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="text-left text-xs font-bold uppercase tracking-wide text-gray-500">
                      ...
                      <th className="px-3 py-2">Tasks Created</th>
                      <th className="px-3 py-2">Class Average</th>
                  ...
                  <tbody>
                    {teachers.map((row) => (
                      <tr key={row.id} className="border-t border-gray-100">
                        ...
                        <td className="px-3 py-3">{row.tasks_created_count ?? 0}</td>
                        <td className="px-3 py-3 font-semibold">
                          {formatScore(row.class_average ?? row.average_student_score)}
                        </td>
                        <td className="px-3 py-3 text-center">
                          <Link
                            to={`/school/teachers/${row.id}`}
