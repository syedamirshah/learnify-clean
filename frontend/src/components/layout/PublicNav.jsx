...
export default function PublicNav({ items = [], className = "" }) {
  return (
    <nav className={`hidden w-full border-t border-white/10 bg-[#42b72a] md:flex md:h-16 md:items-center ${className}`}>
      <div className="mx-auto flex h-full w-full flex-1 min-w-0 flex-wrap items-center justify-center gap-x-6 gap-y-1 px-4 md:flex-nowrap md:justify-between md:gap-x-0 lg:px-6">
        {items.map((item) => (
